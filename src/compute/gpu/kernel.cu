...
// Core: build RNG state from a precomputed z = seed + index * GOLDEN64 (mod 2^64).
__device__ __forceinline__ void make_rng_z(uint64_t z, uint32_t s[4]) {
...
// Convenience wrapper kept for clarity / external use; bit-identical to make_rng_z.
__device__ __forceinline__ void make_rng(uint64_t seed, uint64_t index, uint32_t s[4]) {
    make_rng_z(seed + index * UINT64_C(0x9e3779b97f4a7c15), s);
}
...
void bogo_shuffle_kernel(
    ...
    uint32_t           min_threshold,        // floor(moving_avg_score) - 1; pass 0 if unknown
    uint32_t           *global_threshold     // [1] shared across all blocks, init to min_threshold
) {
    ...
    // Initialised to min_threshold so threads prune aggressively from the start
    // based on the recent moving average of best scores.
    __shared__ uint32_t block_threshold;

    if (threadIdx.x == 0) block_threshold = min_threshold;
    __syncthreads();
    ...
    // Incremental splitmix64 seed: z(idx) = seed + idx * GOLDEN64 (mod 2^64).
    // Since idx advances by `stride` each iteration, z advances by a fixed
    // z_step = stride * GOLDEN64 each iteration. Addition is exact (mod 2^64
    // wraparound is a ring), so z_base at iteration c is bit-identical to
    // computing seed + (t_start + c*stride) * GOLDEN64 from scratch. This
    // trades a 64x64-bit multiply per candidate for a 64-bit add.
    const uint64_t GOLDEN64 = UINT64_C(0x9e3779b97f4a7c15);
    const uint64_t z_step   = (uint64_t)stride * GOLDEN64;
    uint64_t z_base = seed + t_start * GOLDEN64;
    ...
    for (uint32_t c = 0; c < my_iters; c++) {
        ...
        { uint32_t g = *global_threshold; if (g > threshold) threshold = g; }
        ...
        uint32_t s[4];
        make_rng_z(z_base, s);
        z_base += z_step;  // advance unconditionally, regardless of continue/break below
